...
class MLP(nn.Module):
  ...
  def __call__(self, x, condition=None):
    ...
    for i in range(self.net_depth):
      ...
      if i % self.skip_layer == 0 and i > 0 and i != (self.net_depth - 1):
...
class full_MLP(nn.Module):
  ...
  def __call__(self, x, it, pretrain, condition=None, prob=None, rng=None):
    ...
    # for fixing selector
    # pretrain: allow gradient from ent update selector
    if pretrain is None:
      pretrain = 0.
    else:
    ...
    hard_prob = pretrain * jax.nn.one_hot(hard_idx, soft_prob.shape[-1]) + (1. - pretrain) * jax.nn.one_hot(prob_bkup.argmax(-1), soft_prob.shape[-1])
