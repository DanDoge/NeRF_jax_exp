...
def train_step(model, rng, state, batch, lr):
  ...
  def loss_fn(variables):
    ...
    if len(ret) > 1:
      ...
      loss_f = ((rgb_c - batch["pixels"][Ellipsis, :3])**2).mean()
      rgb_f2, unused_disp_f2, unused_acc_f2 = ret[2]
      loss_f2 = ((rgb_c - batch["pixels"][Ellipsis, :3])**2).mean()
    ...
    stats = utils.Stats(
        loss=loss, psnr=psnr, loss_c=loss_c, loss_f=loss_f, loss_f2=loss_f2, psnr_c=psnr_c, weight_l2=weight_l2)
    return loss + loss_c + loss_f + loss_f2 + FLAGS.weight_decay_mult * weight_l2, stats
  ...
  for step, batch in zip(range(init_step, FLAGS.max_steps + 1), pdataset):
    ...
    if jax.host_id() == 0:
      if step % FLAGS.print_every == 0:
        ...
        summary_writer.scalar("train_loss_fine2", stats.loss_f2[0], step)
